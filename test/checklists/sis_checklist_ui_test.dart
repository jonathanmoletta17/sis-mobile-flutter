...
void main() {
  group('catalog screen gate (perfil OR grupo)', () {
    ...
    testWidgets(
      'Operador Manutencao (profile 11 + grupo 22) ve via group_ids',
      (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: SisChecklistCatalogScreen(
  ...
  group('catalog screen tipo (PREVENTIVA/CORRETIVA)', () {
    ...
    testWidgets(
      'default e PREVENTIVA; ao mudar para CORRETIVA repassa para form',
      (tester) async {
        String? capturedType;
        await tester.pumpWidget(
          MaterialApp(
  ...
  group('form screen', () {
    testWidgets(
      'allows review when target pre-fills required field and conditional is hidden',
      (tester) async {
        ...
        expect(
          ...
          isNotNull,
          reason: 'review enabled quando required visivel preenchida',
        );
        expect(
          find.text('Detalhe'),
          findsNothing,
          reason: 'campo condicional oculto inicialmente',
        );
      },
    );

    testWidgets('blocks review when required visible field is empty', (
      tester,
    ) async {
      ...
      expect(
        reviewButton.onPressed,
        isNull,
        reason: 'review disabled com required visivel vazia',
      );
    });

    testWidgets(
      'pre-fill: target conditions pre-select Local=A via multiselect',
      (tester) async {
        ...
        expect(
          checkA.value,
          isTrue,
          reason: 'Local=A deve vir pre-marcado pelo target 341',
        );
    ...
    testWidgets('selecting Local=B reveals the conditional required section', (
      tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(
    ...
    testWidgets('inicializa pergunta Checklist com defaultValue PREVENTIVA', (
      tester,
    ) async {
    ...
    testWidgets('preselectedType passa tipo escolhido na tela de catalogo', (
      tester,
    ) async {
    ...
    testWidgets('glpiselect sem ticketSearcher mostra info box (nao interativo)', (
      tester,
    ) async {
    ...
    testWidgets('glpiselect com ticketSearcher mostra campo interativo', (
      tester,
    ) async {
      ...
      Future<List<Map<String, dynamic>>> fakeSearcher(String q) async =>
          const [];
      ...
      await tester.pump();
      expect(
        find.byKey(const Key('checklist_glpiselect_field')),
        findsOneWidget,
      );
      expect(find.text('Selecionar chamado...'), findsOneWidget);
    });

    testWidgets('submission disabled shows review button, not send', (
      tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(
    ...
    testWidgets(
      'glpiselect PluginGenericobjectConservacao com conservacaoSearcher mostra seletor interativo',
      (tester) async {
        final catalogComConservacao = SisChecklistCatalog.fromMap({
          ...
          'sections': [
            {'id': 800, 'form_id': 80, 'name': 'Calhas', 'order': 1},
          ],
          'questions': [
            {
              'id': 900,
          ...
          'categories': [
            {
              'id': 148,
              'name': 'Calhas',
              'completename': 'Manutencao > Checklist > Calhas',
              'parent_id': 147,
              'level': 3,
            },
          ],
        ...
        Future<List<Map<String, dynamic>>> fakeConservacaoSearcher(
          String q,
        ) async => const [];
        ...
        await tester.pump();
        expect(
          find.byKey(const Key('checklist_conservacao_900')),
          findsOneWidget,
        );
    ...
    testWidgets(
      'conservacaoResolver pre-popula campo com nome resolvido assincronamente',
      (tester) async {
        final catalogComDefault = SisChecklistCatalog.fromMap({
          ...
          'sections': [
            {'id': 810, 'form_id': 81, 'name': 'Calhas', 'order': 1},
          ],
          'questions': [
            {
              'id': 901,
        ...
        Future<List<Map<String, dynamic>>> noopSearcher(String q) async =>
            const [];
        Future<String?> fakeResolver(int id) async =>
            id == 42 ? 'CALHAS ALA GOVERNAMENTAL' : null;
    ...
    testWidgets('multiselect com default_values=[] nao pre-preenche o campo', (
      tester,
    ) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: SisChecklistFormScreen(
            catalog: catalogComDefaultVazio,
            formId: 60,
            targetId: 500,
          ),
        ),
      ...
      await tester.pump();
      final checkX = tester.widget<CheckboxListTile>(
        find.byKey(const Key('checklist_check_X')),
      );
      final checkY = tester.widget<CheckboxListTile>(
        find.byKey(const Key('checklist_check_Y')),
      );
      expect(
        checkX.value,
        isFalse,
        reason: 'X nao deve estar marcado — default_values=[] e vazio',
      );
      expect(
        checkY.value,
        isFalse,
        reason: 'Y nao deve estar marcado — default_values=[] e vazio',
      );
      final reviewButton = tester.widget<FilledButton>(
        find.byKey(const Key('checklist_review_button')),
      );
      expect(
        reviewButton.onPressed,
        isNull,
        reason: 'review bloqueado — campo required vazio',
      );
    });

    testWidgets('condicoes de target vencem sobre default_values nao vazio', (
      tester,
    ) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: SisChecklistFormScreen(
            catalog: catalogComDefaultNaoVazio,
            formId: 70,
            targetId: 600,
          ),
        ),
      ...
      await tester.pump();
      final check951 = tester.widget<CheckboxListTile>(
        find.byKey(const Key('checklist_check_951')),
      );
      final checkAR = tester.widget<CheckboxListTile>(
        find.byKey(const Key('checklist_check_AR CENTRAL')),
      );
      final checkCC = tester.widget<CheckboxListTile>(
        find.byKey(const Key('checklist_check_CASA CIVIL')),
      );
      expect(
        check951.value,
        isTrue,
        reason: '951 deve estar marcado pela condicao do target',
      );
      expect(
        checkAR.value,
        isFalse,
        reason:
            'AR CENTRAL nao deve estar marcado — condicao de target vence sobre default',
      );
      expect(
        checkCC.value,
        isFalse,
        reason:
            'CASA CIVIL nao deve estar marcado — condicao de target vence sobre default',
      );
    });
  });
}
