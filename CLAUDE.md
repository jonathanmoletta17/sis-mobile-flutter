...
- Contas de teste como atores universais de validacao (2026-07-02, substitui a versao anterior com UMA conta unica): existem 4 contas de teste dedicadas (login proprio de QA/bot, credenciais em `.env` nao versionado) — `sis-teste-solicitante`, `sis-teste-manutencao`, `sis-teste-conservacao`, `sis-teste-gg` — usadas para TODA validacao mutavel necessaria no GLPI real. Usuarios reais (producao) sao SEMPRE somente leitura, usados so para comparacao/validacao contra o comportamento esperado; nunca sofrem mutacao de nenhum tipo. Claude/agentes nunca operam como usuario real e nunca pela sessao de servico elevada do Worker (essa permanece restrita a GET de diretorio User/Group).
- Fronteira de seguranca: nao e "qual credencial", e "qual usuario e alvo". Qualquer uma das 4 contas de teste pode ser mutada livremente (de forma nao-destrutiva) tanto pela sessao da propria conta quanto pelas credenciais administrativas de teste — nunca por usuario real, sob nenhuma credencial.
- Criar chamado e mutacao NAO-destrutiva e reversivel. Claude/agentes PODEM criar tickets de teste no GLPI real (incl. producao) via `POST /Ticket` e agir SOMENTE sobre os tickets criados na mesma execucao (`PUT` de status, followup, solucao, anexo), sob TODAS as condicoes abaixo:
  - Ator padrao: a propria conta de teste do papel sendo validado (ex.: `sis-teste-solicitante` para fluxo de Solicitante). Admin de teste entra como FALLBACK DECLARADO quando a conta de teste nao tem o direito necessario para a acao ou para a leitura de verificacao do efeito (ex.: perfil sem `READALL` nao consegue reler `Document_Item`/status de ticket que criou mas nao esta atribuido a si — achado real, nao hipotetico, documentado em memoria) — nunca como atalho de conveniencia quando a conta de teste teria conseguido sozinha.
  - Marcacao obrigatoria: todo ticket de teste leva prefixo identificavel no titulo (ex.: `[TESTE-AUTOMATIZADO SIS]`) e conteudo deixando claro que e teste automatizado descartavel. GLPI recusa renomear ticket ja Fechado por perfil sem UPDATE amplo — definir a marcacao ANTES de criar (ex.: via `SIS_CHECKLIST_TICKET_NAME_PREFIX` ou equivalente), nao depois.
  - Cleanup e auditoria: apos validar, encerrar/cancelar o proprio ticket de teste e registrar o ID criado para limpeza humana e auditoria. Se o fechamento nao puder ser confirmado por escopo de leitura da conta usada, registrar isso explicitamente em vez de presumir sucesso.
  - Escopo fechado: nunca executar mutacao em ticket cujo requerente/criador nao seja uma das contas de teste; nunca tocar tickets de usuarios reais, sob nenhuma credencial.
- Simulacao de papeis: para validar fluxos de Solicitante, Tecnico/Conservacao, Tecnico/Manutencao, observador e demais papeis, e permitido alternar o PERFIL ATIVO (`changeActiveProfile`) e a ENTIDADE ATIVA da sessao de qualquer conta de teste entre os perfis/entidades atribuidos a ela. Esse caminho exige GLPI direto (rede interna/VPN), pois o Worker nao expoe `changeActiveProfile`.
- Ajuste de atribuicoes das contas de teste: conceder/revogar perfis (`Profile_User`), grupos (`Group_User`) e entidades e permitido sobre qualquer uma das 4 contas de teste, de forma reversivel e registrada, revertendo ao estado original ao fim da validacao — inclui perfil, entidade E grupo, nao so os dois primeiros. Relevante em ambiente com multiplas sessoes paralelas usando as mesmas contas: reverter evita quebrar o estado que outra sessao esta usando. Exige credenciais administrativas de teste fornecidas explicitamente para esse fim (em `.env`), nunca a sessao de servico do Worker. PROIBIDO alterar atribuicoes, perfis, grupos ou entidades de qualquer usuario real.
- Variaveis em `.env` (nao versionado): `SIS_TEST_SOLICITANTE_USER`/`PASSWORD`, `SIS_TEST_MANUTENCAO_USER`/`PASSWORD`, `SIS_TEST_CONSERVACAO_USER`/`PASSWORD`, `SIS_TEST_GG_USER`/`PASSWORD` para as 4 contas de teste; `SIS_TEST_ADMIN_USER`/`PASSWORD` para o fallback administrativo de teste. O harness de validacao e read-only por padrao; muta so com flag explicita (`SIS_VALIDATION_APPLY=true`).
- Proibicao absoluta, independente de identidade, credencial ou ambiente: `DELETE /Ticket`, `DELETE`/purge de usuarios, grupos ou entidades, cleanup destrutivo automatizado e qualquer metodo destrutivo via Worker SIS pass-through permanecem vetados sem aprovacao humana explicita e ambiente isolado. Isso vale igualmente para as 4 contas de teste — "pode mutar a vontade" nunca inclui destruir.
